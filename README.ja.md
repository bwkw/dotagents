...
./scripts/test-verify-gate.sh   # 検証ゲートの 42 テスト
./scripts/test-setup.sh         # インストーラの 14 テスト（偽の HOME に対して）
```
...
`setup.sh` はテンプレートが宣言したキーだけをマージし、書いた内容を `~/.claude/.dotagents-managed.json` に記録し、編集前にバックアップを取り、既存の値は読みも書き換えもしません（エージェント設定に既に入っている資格情報を含む）。既存のフックは置換ではなく追記します。`./scripts/setup.sh uninstall` は追加したぶんだけを正確に取り消します。

1点、以前の記述が過大だったので明記します: revert が戻すのは各キーの**値**であって、ファイルの**バイト列**ではありません。設定は2スペース固定の整形で書き直されるので、別の整形をしていたファイルは整形され直して戻ります。`scripts/test-setup.sh` が検証しているのは値であり、それが実際に約束できることです。
