...
`/da-investigate` は ● `da-codebase-explorer` サブエージェントに展開するので、**読んだ内容は彼らの context に留まります**。
...
`da-review-backend` / `-frontend` / `-infra` は完全なスキルですが **`/` メニューには出しません** —— ディスパッチャが届き、層を名指しした依頼（「backend をレビューして」）も届きます。**打つのは1つ、その裏に3層の深さ。**
