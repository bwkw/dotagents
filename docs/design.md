...
- **コストの可観測性が、まだ半分しかありません。** OpenTelemetry はスキル発火イベント用に設定してありますが、トークンや金額を追うものが何もありません。このギャップは実際に刺さって、`/da-review-all` が3層で subagent 約69本を立てていたことに体感で気づくまで誰も測っていませんでした（決定16）。**今あるのは自己申告だけです** —— レビュー報告の 🔎 が予算段階と subagent 本数を書きます。安いので入れましたが、これは計器ではなく**モデルが自分で数えた数字**で、間違っていても誰も気づきません。ハーネス側から実測を取るものが要る。
