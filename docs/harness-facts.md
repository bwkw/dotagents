...
- **`POST /pulls/{n}/reviews` が `event: COMMENT` で空の `body` を拒否するか。** REST のドキュメントは
  `COMMENT` / `REQUEST_CHANGES` で body 必須と読めますが、**実測していません** —— 実走では全所見が inline
  だった PR に1行の body を書いて回避したので、空で投げた場合の応答を見ていない。`pr-comments.md` は
  「1行書け」という規則だけを持ち、API の要件としては書いていません。確認するには他人の PR に空 body を
  投げることになるので、**優先度は低いままにします**
