...
1. **Three entry points disappeared.** `reference/backend.md` is not invokable. "Review just the
   backend" had to go through `/da-review-all` and its classification step, which is exactly the work you
...
A fourth, smaller problem: `AGENTS.md` claimed `da-review-all` "dispatches to its layer references by
...
- `da-review-all` no longer symlinks `review-process.md`, `verification.md`, or `report-format.md`; it
