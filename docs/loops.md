...
### ゲートが landing の壁時計を支配している —— [`docs/decisions.md` §29](decisions.md)

**測ってあります。** docs だけの周は約 3.0s、`scripts/loop.sh` を触る周は約 600s（`loop-tests` 単独で
579.6s、全15検査 703.6s の82%）。この節はかつて §29 の本文をそのまま複製していて、**そのせいで
「1周ごとに全検査が走る」という、`paths` 絞り込みが入ったあとは偽になった文が2箇所に残っていました。**
数字と経緯は §29 の1箇所だけにしてあります。
...
**実測は `scripts/loop.sh report` が出します** —— 段別コスト、landing あたりの値段、周回数、停止理由の
内訳、そして過去の行に含まれる二重計上の額まで。**台帳は生きているので、写しを持つと写しの方が古く
なります。** かつてここには実測ログのファイルへのポインタがあり、その中身は report が出すものの
凍った版でした。

**そのファイルが存在した理由の方は残す価値があります。** 実測で分かったことでした: レビュー段のコストが
$1.25 → $2.07 に伸びたのに、依頼は毎回「`docs/loops.md` 1ファイル」で変わっていない。**伸びたのは
このファイル自身**（当時 917行 / 71KB）で、**レビューは変更行だけでなくそれが載っているファイルを
読みます。** 206行の実測ログは、その値段の23%を占めていました。

`SKILL.md` の「12KB 以下、詳細は `reference/` へ」（`AGENTS.md`）と同じ理由が docs にも効く、という
ことです。**追記されるだけの履歴を、読まれるファイルに置かないこと。** 置き場所が別ファイルではなく
台帳になったのは、そちらが自分で更新されるからです。
