...
**これは書式の例で、実データではありません。** 実走は7回で `reached PR` は **0** です（下の
「実走7回の停止理由 —— 中心と外周」）。`landings attempted` の 7 が偶然一致しているだけです。
...
**7回走って、7回止まりました。** 折れた場所で分けると **中心**（この駆動系が決めていること —— レビューの
コスト・停止条件・上限の順序）と **外周**（他の道具の振る舞い）になり、**中心が最後に折れたのは4回目です。
5回目以降の3回は、全部外周でした。**（これを書いているのは8回目の実走です。）
...
### 中心の3項目は、それぞれ折れて、直って、再発していません

- **レビューのコスト** —— 最後に止めたのは2回目。3回目以降は $1.25 / $1.22 / $1.40 / $0.88 / **$1.43**
  （20 / 20 / 28 / 15 / 23 turns）で、**5回とも天井 $2.00 の下**。上の「レビュー段の実測」表は6点までで、
  **7回目の $1.43 / 23t はまだ載っていません**（表の更新はこの landing の対象外）。
...
- **上限の順序** —— 最後に止めたのは4回目。5・6・7回目はいずれも triage → fix を通り抜けて `pr` / `ci` 段
  まで届いています。**`review_cap` はもう `loop.sh` に存在しません**（`grep review_cap scripts/loop.sh` が
...
- **CI の3つ（と #49 が足した `CI_GRACE_SECONDS` (120)）は、いまも測定値を持っていません。** ただし
  ...
  値段」ではなく「無い赤を探した1周の値段」です（上の「実走7回の停止理由」）。`CI_WAIT_SECONDS` と
...
- **採択1件あたりのコストは、まだ出ていません** —— そして**理由は予算ではなくなりました。止まる場所が
  奥へ移っています。** 台帳の7 landing の停止理由は、古い順に
  `budget` → `budget` → `needs_decision` → `review_cap` → **`pr_failed`** → **`push_failed`** →
  **`ci_fix_changed_nothing`**（内訳は上の「実走7回の停止理由 —— 中心と外周」）。
  最後の3回は review と triage と fix を通り抜けていて、**落ちたのはレビューの内容ではなく外周の
  道具**です。#46（存在する PR を「失敗」と報告していた）と #47（マージでリモートがブランチを消すと
  push できない）の修正は **7回目で初めて通り抜けました** —— push が通り、PR の URL も解決した。
  そして同じ run が次の1つに当たっています（#49、`gh pr checks` の誤読）。
  つまり `review_cap` までの4回は中心の欠陥で、**直近3回は外周**です。どちらも「ループは正しく
