...
- **S / M / L の閾値と `MAX_ROUNDS` / `REVIEW_ROUNDS` / `BUDGET_USD` / `MAX_OPEN_PRS` /
  `ROUND_TIMEOUT` と、round ごとの `BUDGET_ROUND_*` —— 今日足した `BUDGET_ROUND_CI` ($2.00) /
  `CI_ATTEMPTS` (2) / `CI_WAIT_SECONDS` (900) もここに並びます。** 全部**選んだ数字**で、
...
- **CI の3つは「まだ測っていない」より一段悪く、まだ一度も走っていません。** 台帳に載っている phase は
  `size` / `design` / `implement` / `review` / `triage` の21行だけで、**`ci` の行は1本もありません** ——
  PR まで届いた run がまだ無いからです（下記）。レビューの天井は少なくとも「実測 $5.64 / $6.19 より下、
  brief 形式が要る額より上」に挟んで置きましたが、**この3つは上も下も空です。**
  外したときに何を買うかだけは、いま書けます:
