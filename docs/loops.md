...
- **CI の3つは「まだ測っていない」より一段悪く、まだ一度も走っていません。** 台帳には `fix` と `pr` の
  行が増えました（PR 段まで届いた landing が2つ）。それでも **`ci` の行は1本も
  ありません** —— PR が開くところまで行った run がまだ無いからです（下記）。`findbugs` も同じくゼロで、
  こちらは理由が別です: 測った landing の `risk_surfaces` が毎回 0 で、2本目のレビュアを買う条件に
  一度も当たっていません。レビューの天井はいま修正後の実測4点の上に載っていますが、**この3つは上も下も
  空のままです。** 外したときに何を買うかだけは、いま書けます:
...
- **採択1件あたりのコストは、まだ出ていません** —— そして**理由は予算ではなくなりました。止まる場所が
  奥へ移っています。** 台帳の6 landing の停止理由は、古い順に
  `budget` → `budget` → `needs_decision` → `review_cap` → **`pr_failed`** → **`push_failed`**。
  最後の2回は review と triage と fix を通り抜けて `pr` 段まで届いていて、**落ちたのはレビューの
  内容ではなく stack の機構**です。同じ形の失敗にはその後それぞれ修正が入っていますが
  （#46「存在する PR を『失敗』と報告していた」・#47「マージでリモートがブランチを消すと push
  できない」）、**それを通り抜けた実走はまだありません。**
  つまり `review_cap`（設計どおりの停止）までは「ループは正しく動いていて指標だけが埋まっていない」で
  したが、**直近2回はそうではありません** —— 埋まっていないのは欠陥のためで、修正済みかどうかは
  次の実走が言います。`cost per accepted` が `--` の間は、**計器は動いても測定は済んでいない**と
  読んでください。
