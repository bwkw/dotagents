...
| `round_errored` | その周が**エラーを返した** —— `is_error: true`、または `error_max_*` ではない subtype。**天井ではありません。** この2つは長いあいだ同じメッセージで、`is_error: true` かつ `subtype: "success"` の応答が「天井で切られた（subtype: success）」と報告され、**その周の天井を上げろ**と助言していました —— **`implement` には天井が無い**（`BUDGET_ROUND_IMPLEMENT` という定数は存在しない）ので、存在しないつまみを指していたことになります。10回目の実走が $1.2784895 / 24 turns でこの形を出し、台帳には偽の `truncated` が記録されました。検出は変えていません（deny by default）—— **変えたのは分類と助言だけ**で、`error_max_*` は将来 CLI が増やすものも含めて `truncated` 側に残ります |
...
- **XS / S / M / L の閾値と `MAX_ROUNDS` / `REVIEW_ROUNDS` / `BUDGET_USD` / `MAX_OPEN_PRS` /
  ...
  **ここから出たのは1つだけです**: `BUDGET_ROUND_REVIEW_LEAN`（当時 `_S`、$2.00 →いまは $3.00）は
  選んだのではなく測りました
...
- **`BUDGET_ROUND_PR` だけは、天井に当たって測れました** —— そしてそれが、上の2つと違う点です。
  2026-08-19 の初の端到達（tier XS、無人）で `/da-pr-describe` の周が **$1.50 に対し $1.55** で切られ、
  **PR はテンプレートが未記入のまま開きました**（要約なし・検証なし・何も主張していないチェックボックス）。
  `$2.50` に上げましたが、**1点で数字は選べません** —— 根拠は数字ではなく**非対称性**です: 低すぎる側は
  本文が丸ごと失われたうえ landing は止まらず（`opened-pr-partial-body` は停止しない）、高すぎる側は
  金だけで、しかも `truncated` が声を上げるので見えます。値は $1.55 に足したのではなく、**同じ種類の
  仕事である `BUDGET_ROUND_COMMENTS` ($2.50) から取りました。**
