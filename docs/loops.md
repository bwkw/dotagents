...
台帳（34行）から、天井のある残りの2段を取り出したもの。**`size` はこの文書が「次に削るべき場所」と
名指しした段**です。
...
- **S / M / L の閾値と `MAX_ROUNDS` / `REVIEW_ROUNDS` / `BUDGET_USD` / `MAX_OPEN_PRS` /
  ...
  **ここから出たのは1つだけです**: `BUDGET_ROUND_REVIEW_S` ($2.00) は実測6点の散らばりの上に
  置いた数字で、選んだのではなく測りました（上の「レビュー段の実測」）。
...
- **CI の3つは「まだ測っていない」より一段悪く、まだ一度も走っていません。** 台帳は34行になり、
  `fix` と `pr` の行が増えました（PR 段まで届いた landing が2つ）。それでも **`ci` の行は1本も
  ありません** —— PR が開くところまで行った run がまだ無いからです（下記）。`findbugs` も同じくゼロで、
  こちらは理由が別です: 測った landing の `risk_surfaces` が毎回 0 で、2本目のレビュアを買う条件に
  一度も当たっていません。レビューの天井はいま実測6点の上に載っていますが、**この3つは上も下も空の
  ままです。** 外したときに何を買うかだけは、いま書けます:
