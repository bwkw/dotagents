...
| `truncated` | その周が自分の天井（`--max-budget-usd`）で切られた。**答えは部分的で、下流はそれを完成した報告として扱ってはいけません。** 天井超過は exit 1 で返るので `round_failed` より**先に**見ています（上記）—— 逆順だった版では、これが作られた目的そのもののケースで一度も発火しませんでした。天井を上げるかその周を安くするかで、切れた報告を綺麗な結果と読むのは違います |
...
| `ci_red` | `CI_ATTEMPTS`（既定 2）回直しても CI が赤いまま。**それ以上は失敗した方法を積み上げます** —— 上限に達した最後の1周が報告専用で修正しないのは、そのためです |
