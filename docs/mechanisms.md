...
以下は作者のマシンの話ではなく、**どのマシンでも起こる形**です。実測値は1台から取っているので、数字は例として読んでください。

**Claude Code が2つ入っていることがあり、設定の効き方は同じではありません。** バージョン管理ツール（mise、asdf、nvm など）の shim 経由で `$PATH` に入る `claude` と、Claude Desktop が自前でダウンロードして動かすバイナリは、**別物で、別のバージョンでありうる**。片方だけを見て設定を書くと、もう片方では黙って無視されます。

作者のマシンで両バイナリの設定スキーマを直接読んで確認した例（`$PATH` 側が 2.1.148、Desktop 側が 2.1.219 だったとき）:
...
**自分のマシンで確認する方法:** `which -a claude` で shim と実体を並べ、それぞれの `--version` を比べてください。異なるなら、設定を書く前にどちらが読むのかを決める必要があります。
...
| **大きい差分でレビューは緩やかに劣化せず、モデルは一貫性を失って「スタイルのパターン照合」に落ちる。** 1,000行が目安 —— つまり出力は「徹底したレビューに見えて重要な所見を1つも含まない」形になる | [AI Code Review: Approaches, Tools, and Best Practices (2026)](https://collinwilkins.com/articles/ai-code-review-best-practices-approaches-tools) |
| **自動レビュアの偽陽性は 5〜15%**。信頼の指標は**指摘の採択率**で、50%未満は信用されず迂回され、80%超えると soft gate に載せられる水準 | [AI Code Review Practices (2026)](https://blog.exceeds.ai/ai-code-review-best-practices/) · [Sonar](https://www.sonarsource.com/resources/library/best-ai-code-review-tools/) |
| **advisory から始め、hard gate は決定的な規則にだけ。** 偽陽性の高い hard gate は素早く迂回される | 同上 |
...
| **subagent は親のキャッシュ済み prefix を継承しない。** 毎回 cold で立ち上がり、読んだものすべてに uncached 価格を払う。cache write は基本価格の +25%、cache read は 10% —— つまり同じ参照ファイルを N 本のファンアウトで読むと、最も高いレートで N 回買い直している | [Claude Code Sub-Agents: The Hidden Token Cost](https://extraheadroom.com/blog/claude-code-subagents-token-costs) · [How Prompt Caching Actually Works in Claude Code](https://www.claudecodecamp.com/p/how-prompt-caching-actually-works-in-claude-code) · [Anthropic: prompt caching](https://www.anthropic.com/news/prompt-caching) |
| **ファンアウトの実測倍率は 2.6〜5.9×**（Opus で2本 2.6× / 5本 3.2×、Fable 5 で2本 5.9×）。cache 率込みの価格加重では2本で約5倍。そして**速くならない** —— 逐次 4:15 に対し2本 8:00、5本 4:45。「安いモデルに固定すると約37%のトークン削減」「cold なファンアウトは時間差で出す」 | [The Subagent Tax (Systima)](https://systima.ai/blog/subagent-tax) |
| **エージェント数のスケーリングは頭打ちする。** 多エージェントコードレビューの F1 は **n=5〜10 で plateau**。同質なエージェント（同一モデル・同一プロンプト）では1本あたりの限界利得がゼロに向かって崩れる —— 似た分布で訓練されたモデルは同じ間違いに収束し、合議は少数の正解を濾し取る。**観点クラスタの上限を5本に置いた根拠** | [Understanding Agent Scaling via Diversity](https://arxiv.org/html/2602.03794v1) · [Multi-Model AI Code Review: Convergence Loops](https://zylos.ai/research/2026-03-01-multi-model-ai-code-review-convergence/) |
| 多エージェントのコストは**6経路で複利になる**: context 複製 / orchestration overhead / 通信路の組合せ増（5体で10経路、10体で45経路）/ retry での全履歴再課金 / **検証の積み増し（逐次比 約2.3×）**/ 長時間実行。逐次チェーンは調整コストが線形、star・mesh は超線形。緩和は「最小限の context 伝播」「複雑さによるモデルルーティング」「階層的な予算上限」 | [Multi-Agent Cost Compounding (Augment Code)](https://www.augmentcode.com/guides/multi-agent-cost-compounding) |
| Anthropic 自身の多エージェント研究システムは chat の**約15倍**のトークンを使い、**トークン使用量だけで性能分散の80%**を説明する。つまり多エージェントが勝つのは**タスクが独立並列に分解できるときだけ** | [How we built our multi-agent research system](https://www.anthropic.com/engineering/built-multi-agent-research-system) |
