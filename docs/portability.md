...
| # | 足りないもの | 状態 |
|---|---|---|
| 1 | **スキル本文の完全性ゲート** | **未着手。** 設計は済んでいますが、3つの分岐（他人のスキルまで対象にするか / 無限後退をどこで止めると宣言するか / manifest スキーマを変えるか）が未決。決める前に実装すると、決定が実装に引きずられる |
| 2 | **`main` のブランチ保護** | **完了。** ruleset で PR 必須・CI 4本必須・force push 禁止・削除禁止（下記「GitHub 側の状態」） |
