...
> **実走しました。まだ端まで閉じていません。** 台帳には複数日の実走が入っていて、`size` から `ci` まで
> 各段が実際に走っています —— **仕上がった PR に到達した run は1本もありません**（`opened-pr` の行が
> 台帳に1つも無い）。**`findbugs` の段は一度も走っていません。**
>
> 止まった理由は台帳が持っています —— `budget` / `needs_decision` / `review_cap` / `pr_failed` /
> `push_failed` / `ci_fix_changed_nothing` / `round_failed`。**壊れて止まったものと、止めるべくして
> 止まったものが混ざっています。**
>
> **数字はここに写しません。** 走るたびに変わるので、写した時点から古くなります ——
> `scripts/loop.sh report` が今の内訳を、`bash scripts/check.sh` が今のテスト結果を出します。
> この段落は「まだ一度も実走していません／105件緑」と書いたまま、**複数日の実走に追い越されて**
> いました。**計器を作ったことは測ったことではなく、測ったことは閉じたことでもありません。**
